...
data = open('livro.txt', "r", encoding = "utf-8")
...
for capitulo in capitulos:
    textoNaoFormatado = texto.split('CAPITULO')[capitulos.index(capitulo) + 1].strip()
    textoParagrafado = textoNaoFormatado.split(capitulo)[-1].strip()
    livro['CAPITULO ' + capitulo] = ' '.join(textoParagrafado.split('\n'))
...
with open('QuincasBorbas.json', 'w', encoding='utf8') as json_file:
    json.dump(json_data, json_file, ensure_ascii=False)

'''
livro_facil = livro.copy()
for _, key in enumerate(livro_facil):
    print(livro[key])
'''